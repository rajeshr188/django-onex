...
class ChitForm(forms.ModelForm):
    members = forms.ModelMultipleChoiceField(
        widget=Select2MultipleWidget, queryset=Contact.objects.all()
    )
    ...
    def clean(self):
        ...
        if len(m) > self.cleaned_data["member_limit"]:
            raise forms.ValidationError(
                "Only {} members are ALlowed to be added".format(
                    self.cleaned_data["member_limit"]
                )
            )
...
class AllotmentForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if Allotment.objects.filter(
            created__month__gte=c.month, created__year__gte=c.year
        ).exists():
            raise forms.ValidationError(
                "Only One ALlotment Per Month Sorry!\n Chit for {} th month is alloted".format(
                    c.month
                )
            )
